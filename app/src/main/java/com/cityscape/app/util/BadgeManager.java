...
public class BadgeManager {
    ...
    public static void awardBadge(Context context, String userId, String badgeId, String name, String desc, String requirement, String icon) {
        ...
        new Thread(() -> {
            ...
            if (badge == null) {
                badge = new com.cityscape.app.model.UserBadge(userId, badgeId, name, desc, icon, requirement);
                badge.unlock();
    ...
    public static void checkVisitBadges(Context context, String userId, String placeType) {
        ...
        if (hour < 9) {
            awardBadge(context, userId, "early_bird", "Early Bird", "Ai vizitat locații înainte de 9 dimineața!", "Vizitează orice locație între orele 05:00 și 09:00", "ic_badge_early");
        } else if (hour >= 22) {
            awardBadge(context, userId, "night_owl", "Night Owl", "Ești un explorator nocturn!", "Vizitează orice locație după ora 22:00", "ic_badge_night");
        }

        if (placeType != null) {
            if (placeType.toLowerCase().contains("restaurant")) {
                 awardBadge(context, userId, "foodie", "Food Critic", "Ai început să explorezi gastronomia locală!", "Vizitează cel puțin un restaurant recomandat", "ic_badge_generic");
            } else if (placeType.toLowerCase().contains("cafe") || placeType.toLowerCase().contains("cafenea")) {
                 awardBadge(context, userId, "caffeine", "Caffeine Addict", "Energie pură din cele mai bune cafenele!", "Vizitează o cafenea de specialitate", "ic_badge_coffee");
            }
        }
    }

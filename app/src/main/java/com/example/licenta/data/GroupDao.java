...
public interface GroupDao {
    // Activity Groups
    @Insert
    long insertGroup(ActivityGroup group);
    ...
    ActivityGroup getGroupById(int groupId);
    ...
    ActivityGroup getGroupForActivity(int activityId);

    @Query("SELECT ag.* FROM activity_groups ag INNER JOIN group_members gm ON ag.id = gm.groupId WHERE gm.userId = :userId")
    List<ActivityGroup> getGroupsForUser(int userId);

    // Group Members
    @Insert
    long insertMember(GroupMember member);
